...
export default function IntentPanel({ onStarted }) {
  ...
  const [result, setResult] = useState(null); // { first_action, plans }
  ...
  async function handleSubmit(e) {
    ...
    setState("generating");
    try {
      const res = await decompose({ goal: goal.trim(), answers: answers.trim() });
      setResult(res);
      setState("preview");
    } catch (err) {
  ...
  async function handleStart() {
    const sessionIndex = Number(localStorage.getItem("rethread_session_count_v1") || "0");
    ...
    try {
      ...
      console.error("Assignment failed; using deterministic local assignment:", err);
    }

    localStorage.setItem("rethread_session_count_v1", String(sessionIndex + 1));
  ...
  if (state === "preview" && result) {
    // _fallback:true is a valid, honest response, not an error —
    // deliberately not special-cased in the UI. Same look either way.
    return (
      <div style={cardStyle}>
              ...
              >
                {result.plans.map((p, i) => (
                  <div
                    ...
                    style={{
                      padding: "12px 0",
                      borderBottom: i < result.plans.length - 1 ? "1px solid #E4E1D7" : "none",
                      fontSize: 13.5,
                      lineHeight: 1.5,
                    }}
                  >
  ...
  return (
    <div style={cardStyle}>
      ...
      <p style={{ fontSize: 14, color: "#5C5A4E", margin: "0 0 20px", lineHeight: 1.5 }}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <textarea
          ...
          placeholder="Where does it live, and when's it due? (optional)"
          rows={2}
